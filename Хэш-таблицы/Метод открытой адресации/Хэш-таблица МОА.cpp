...
void addEl(client* arr, int n) {
    ...
    cout << "Введите дату рождения счета: ";
    getline(cin, temp1);
...
void hashta(client* arr, int n, hashtab* hashtable) {
    ...
    for (int i = 0; i < n; i++) {
        cout << hashtable[i].number << ' ' << hashtable[i].date << endl;
    }
    cout << endl;
...
void hashFound(hashtab* hashtable, int n, client* arr) {
    ...
    if (hashtable[k].date == a) {
        ...
        for (i = 0; i < n && !found2; i++) {
            if (arr[i].date == a) {
                found2 = TRUE;
                cout << arr[i].FIO << ' ' << arr[i].date << ' ' << arr[i].passport << endl << endl;
            }
        }
    }
    else {
...
void DelEL(client* arr, int n) {
    ...
    if (arr == nullptr) {
        ...
        cin.ignore();
        getline(cin, date);
        pos = hash_function(date, n);
    }
    if (date == arr[pos].date) {
        found = 1;
        strcpy(arr[pos].FIO, "");
        strcpy(arr[pos].date, "");
...
int main()
{
    ...
    while (choice != 0) {
        ...
        case 8:
            for (int j = 0; j < n; j++) {
                cout << arr[j].FIO << " " << arr[j].date << " " << arr[j].passport;
            } 
            cout << endl;
            break;
        case 0:
